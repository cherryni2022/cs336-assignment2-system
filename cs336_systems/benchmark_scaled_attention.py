...
class MHA(nn.Module):
    def __init__(self, d_model:int, num_heads:int, max_seq_len:int):
        ...
        self.compiled_attention = torch.compile(scaled_dot_product_attention)
    ...
    def forward(self, x: Float[Tensor, "batch seq d_k"]):
        ...
        # compiled_attention 初始化一次,不用每次forward compile
        attn_output = self.compiled_attention(Q, K, V, self.mask)
