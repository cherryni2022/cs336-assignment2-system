...
import sys
cs336_basics_dir = Path(__file__).parent.parent + "/" + "cs336-basics"
sys.path.append(cs336_basics_dir)
    ...
    with nvtx.range("Warmup"):
        for _ in range(warmup_steps):
            ...
            if not forward_only:
                #loss = outputs.mean()
                optimizer.zero_grad()
                # TODO: target 也同时生成targets
                loss = lossfn(outputs.view(-1, vocab_size), target.view(-1))
                loss.backward()
                optimizer.step()
