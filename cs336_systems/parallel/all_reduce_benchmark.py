...
def all_reduce_dist(rank, world_size,data_size_mb, backend, benchmark_trails, warmup_trails, result_queue):
    ...
    try:
        ...
        logging.info(f"device {rank}/{world_size} run {benchmark_trails} trails, "
        ...
        if rank == 0:
            ...
            logging.info(f"average all reduce time is {final_avg:.2f} ms")
            if result_queue is not None:
                result_queue.put(round(final_avg, 2))
    finally:
        # 确保所有进程到达一致点后再销毁，避免资源泄漏警告
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_size_mb", type=int)
    ...
    for backend in backends:
        ...
        for world_size in test_world_sizes:
            for data_size_mb in test_data_size_mbs:
                logging.info(f"Benchmark {backend} with {world_size} processes and {data_size_mb} MB data")
