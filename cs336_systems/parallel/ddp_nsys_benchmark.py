...
# 1.试验条件 1node 2 GPU
#   针对xl model 对比测试nativeDDP、flatDDP、IndividualDDP、BucketDDP的train耗时与异步通信耗时
# 2.BucketDDP 在不同bucket size下的性能
...
def naive_ddp(rank, world_size, model, train_x, train_y, optimizer, lossfn, 
            ...
            device, model_type):
        logging.info(f"start naive_ddp {rank}/{world_size} warmup")
...
if __name__ == "__main__":
    ...
    logging.info("received results from worker queue")
